...
#define BUFFER_SIZE 360 //缓冲区的长度,可以修改

uint8_t myringbuffer[BUFFER_SIZE] = {0};

static uint16_t validLen;		   //已使用的数据长度
static uint8_t *pHead = NULL;	   //环形存储区的首地址
static uint8_t *pTail = NULL;	   //环形存储区的结尾地址
static uint8_t *pValid = NULL;	   //已使用的缓冲区的首地址
static uint8_t *pValidTail = NULL; //已使用的缓冲区的尾地址
 ...
 * 		  -2:缓冲区没有初始化
 * */
int wirteRingbuffer(uint8_t *buffer, uint8_t addLen)
{
